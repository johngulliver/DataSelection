...
from DataSelection.datasets.cifar10 import plot_cifar10_images
from DataSelection.algorithms.graph import GraphParameters, build_connectivity_graph, label_diffusion
from DataSelection.selection.selectors.label_based import LabelDistributionBasedSampler, LabelBasedDecisionRule
...
class GraphBasedSelector(LabelDistributionBasedSampler):
    ...
    def __init__(self,
                 ...
                 graph_params: Optional[GraphParameters] = None,
                 name: str = 'Graph Based Sample Selection',
                 allow_repeat_samples: bool = False,
                 decision_rule: LabelBasedDecisionRule = LabelBasedDecisionRule.CROSS_ENTROPY,
        ...
        if graph_params is None:
            self.graph_params = GraphParameters(n_neighbors=self.num_samples // 200,
                                                diffusion_alpha=0.95,
                                                cg_solver_max_iter=10,
                                                diffusion_batch_size=self.num_samples // 10,
    ...
    def get_predicted_label_distribution(self, current_labels: np.ndarray) -> np.ndarray:
        ...
        row_sums = current_labels.sum(axis=1)
        input_label_distribution = current_labels / row_sums[:, np.newaxis]
