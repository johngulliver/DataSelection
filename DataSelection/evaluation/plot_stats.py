...
from DataSelection.selection.simulation_statistics import SimulationStatsDistribution
from sklearn.metrics import auc, confusion_matrix, precision_recall_curve, roc_auc_score, roc_curve
from sklearn.manifold import TSNE
from DataSelection.plotting.interactive_plot import plot_latent_interactive
import seaborn as sns

from DataSelection.utils.default_paths import BENCHMARK2_DIR
from DataSelection.utils.generic import create_folder, save_obj


def plot_selected_cases_in_tsne(embeddings: np.ndarray,
                                labels: np.ndarray,
                                points_legend: Optional[List[str]] = None,
                                selected_cases_ids: Optional[List[int]] = None,
                                t_sne_perplexity: int = 20) -> None:
    """
    Plot the selected cases on a t-SNE representation of the embeddings in a interactive plot.
    :param embeddings: embeddings to plot
    :param labels: corresponding labels
    :param points_legend: a string to interactively annotate each sample when clicking on it
    :param selected_cases_ids: ids of the selected cases, marked with a red x on the graph.
    :param t_sne_perplexity: perplexity to use when computing the t-sne representation of the embeddings.
    """
    embeddings_2d = TSNE(n_components=2, perplexity=t_sne_perplexity).fit_transform(embeddings)
    markers = {"selected": (selected_cases_ids, 'red')}
    plot_latent_interactive(embeddings_2d, labels, points_legend, markers=markers)
...
def _plot_mean_and_confidence_intervals(ax: plt.Axes, y: np.ndarray, color: str, label: str, linestyle: str = '-',
                                        ...
                                        use_derivative: bool = False) -> None:
    if use_derivative:
    ...
    label += f" - AUC: {auc_mean:.4f}"
    std_error = np.std(y, axis=0) / np.sqrt(y.shape[0]) * 1.96
    ax.plot(x, mean, color=color, label=label, linestyle=linestyle, linewidth=3.0)
    ax.fill_between(x,
...
def plot_stats_for_all_selectors(stats: Dict[str, SimulationStatsDistribution],
                                 ...
                                 use_derivative: bool = False) -> Tuple[plt.Figure, plt.Axes]:
    """
    ...
    for _stat, color in zip(stats.values(), cycle(colors)):
        for y_attr_name, y_attr_label, linestyle in zip(y_attr_names, y_attr_labels, cycle(linestyles)):
            ...
            _plot_mean_and_confidence_intervals(ax=ax, y=_stat.__getattribute__(y_attr_name), color=color,
                                                label=y_attr_label + _stat.name, linestyle=linestyle,
        ...
        if plot_n_not_ambiguous_noise_cases:
            ...
            ax.text(average_value_attribute.size + 1, average_value_attribute[-1],
                    f"{average_value_attribute[-1]:.2f}", fontsize=fontsize - 2)
...
def plot_stats(stats: Dict[str, SimulationStatsDistribution],
    ...
    fig, ax = plot_stats_for_all_selectors(stats,
                                           ...
                                           ['Mislabelled_cases_'],
                                           x_label='Number of collected relabels on the dataset',
                                           y_label='Number of remaining non-adjudicated mislabelled samples',
                                           legend_loc=1)
    if save_path:
    ...
    fig, ax = plot_stats_for_all_selectors(stats,
                                           ...
                                           ['Ambiguous_cases_'],
                                           x_label='Number of collected relabels on the dataset',
                                           y_label='Number of remaining non-adjudicated ambiguous samples',
                                           legend_loc=1)
    if save_path:
...
def plot_stats_scores(selector_name: str, scores_mislabelled: np.ndarray, labels_mislabelled: np.ndarray,
    ...
    if scores_ambiguous is not None and labels_ambiguous is not None:
    ...
    # Plot reason for mistake (cumulative)
    # for loop_id, _stat in stats.items():
    #     f, ax = plt.subplots()
    #     f.set_size_inches(12, 10)
    #     selection_type = np.array([selection_type.value for selection_type in _stat.selection_type])
    #     data = {selection_type_enum.name: np.cumsum(selection_type == selection_type_enum.value)
    #             for selection_type_enum in SelectionType}
    #     for (name, array), color in zip(data.items(), cycle(colors)):
    #         ax.plot(array, color=color, label=name)
    #
    #     ax.grid()
    #     ax.set_xlabel("Iteration")
    #     ax.set_ylabel("Cumulative sum of selection type")
    #     ax.legend(loc=1)
    #     if save_path:
    #         f.savefig(save_path / f"selection_type_{loop_id}.png", bbox_inches='tight')
    #     else:
    #         plt.show()
