...
import numpy as np
import torch
from DataSelection.deep_learning.self_supervised.ssl_classifier_module import SSLClassifier
...
def register_embeddings_collector(models: List[torch.nn.Module],
    ...
    for model in models:
        ...
        if hasattr(model, "projection") or hasattr(model, "fc"):
            cnn_embeddings.layer = model.projection if hasattr(model, "projection") else model.fc  # type: ignore
