...
from DataSelection.configs import model_config
from DataSelection.configs.selector_config import get_default_selector_config
from DataSelection.deep_learning.self_supervised.configs import ssl_model_config
from DataSelection.deep_learning.self_supervised.utils import create_ssl_image_classifier
from DataSelection.utils.aml import download_checkpoint_to_local
from DataSelection.utils.default_paths import PROJECT_ROOT_DIR, get_train_output_dir
from DataSelection.utils.generic import setup_cudnn
...
def create_logger(output_dir: str) -> None:
    log_path = Path(output_dir).absolute() / 'training.log'
...
def create_model(config: ConfigNode, model_id: Optional[int]) -> torch.nn.Module:
    ...
    if config.train.use_self_supervision:
        ...
        if config.train.self_supervision.run_id is not None and not model_checkpoint_path.exists():
            # If the config specifies an AML run_id download the corresponding checkpoint.
            download_checkpoint_to_local(config.train.self_supervision.run_id, model_checkpoint_name)
        model = create_ssl_image_classifier(config.dataset.n_classes, pl_checkpoint_path=str(model_checkpoint_path))
    else:
        try:
            module = import_module('PyTorchImageClassification.models'f'.{config.model.type}.{config.model.name}')
        except ModuleNotFoundError:
            module = import_module(
                f'DataSelection.deep_learning.architectures.{config.model.type}.{config.model.name}')
