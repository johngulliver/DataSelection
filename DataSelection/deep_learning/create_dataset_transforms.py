...
from DataSelection.deep_learning.transforms import AddGaussianNoise, CenterCrop, ElasticTransform, \
...
def create_transform(config: ConfigNode, is_train: bool) -> Callable:
    if config.dataset.name in ["NoisyKaggle", "COVID", "Kaggle"]:
        return create_chest_xray_transform(config, is_train)
    elif config.dataset.name in ["CIFAR10", "CIFAR10H"]:
