...
from DataSelection.deep_learning.architectures.ema import EMA
from DataSelection.deep_learning.collect_embeddings import register_embeddings_collector
from DataSelection.deep_learning.dataloader import (get_number_of_samples_per_epoch, get_train_dataloader,
                                                          get_val_dataloader)
from DataSelection.deep_learning.metrics.tracker import MetricTracker
from DataSelection.utils.dataset_utils import get_datasets
...
class ModelTrainer(object):
    ...
    def _create_metric_trackers(self, config: ConfigNode) -> Tuple[List[MetricTracker], List[MetricTracker]]:
        ...
        dataset_train, _ = get_datasets(config)
        train_trackers = [MetricTracker(
            dataset=dataset_train,
            output_dir=str(self.log_dir / f'model_{i:d}_train'),
            num_samples_total=len(train_loader.dataset),
        ...
        val_trackers = [MetricTracker(str(self.log_dir / f'model_{i:d}_val'),
                                      num_samples_total=len(val_loader.dataset),
    ...
    def load_checkpoints(self, epoch: Optional[int] = None) -> None:
        """
        ...
        for ii in range(len(self.models)):
            ...
            if self.config.train.restore_scheduler:
    ...
    def run_training(self) -> None:
        ...
        if self.config.train.resume_epoch > 0:
            ...
            self.load_checkpoints(resume_epoch)
    ...
    def run_inference(self, dataset: Any) -> List[MetricTracker]:
        """
        Deployment of pre-trained model.
        """
        dataloader = get_val_dataloader(dataset, self.config, seed=self.config.train.seed)
